...
| ignorePTTorrent | bool | true (启用) | 忽略 PT Torrent. 若主要 Tracker 包含 ```?passkey=```/```?authkey=```/```?secure=``` 或满足 ignorePTTorrentByRandomStr 的条件 |
| ignorePTTorrentByRandomStr | bool | true (启用) | 忽略 PT Torrent (随机字符串检测, 须先启用 ignorePTTorrent). 若主要 Tracker 的路径或查询参数中包含 ```32 位大小写英文及数字组成的随机字符串```, 则视为 PT Torrent |
...
| banByPUStartPercent | float64 | 2 (%) | 增强自动屏蔽/起始进度. 若客户端上传进度大于设置起始进度, 则允许屏蔽 Peer |
...
| banByRelativePUStartPercent | float64 | 2 (%) | 增强自动屏蔽_相对/起始进度. 若客户端相对上传进度大于设置起始进度, 则允许屏蔽 Peer |
