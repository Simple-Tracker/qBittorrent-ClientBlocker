...
| --reghotkey | true | Register window hotkey. Only available on Windows. The window hotkey is fixed to CTRL+ALT+B |
...
| ignorePTTorrent | bool | true | Ignore PT Torrent. If the main Tracker contains ```?passkey=```/```?authkey=```/```?secure=``` or meets the ignorePTTorrentByRandomStr condition |
| ignorePTTorrentByRandomStr | bool | true | Ignore PT Torrent (Random String Detection, must enable ignorePTTorrent). If the main Tracker URL path or query parameters contain ```a 32-char alphanumeric random string```, it is treated as a PT Torrent |
...
| banByPUStartPercent | float64 | 2 (%) | Enhanced automatic blocking/Start progress. If the client upload progress is greater than the set start progress, Peer will be automatically block |
...
| banByRelativePUStartPercent | float64 | 2 (%) | Enhanced automatic blocking_Relative/Start progress. If the relative upload progress of the client is greater than the set start progress, Peer will be automatically block |
